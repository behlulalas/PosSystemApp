...
namespace FastFoodPOS.Forms
{
    public partial class FormLoading : BaseForm
    {
        ...
        {
            System.Threading.Thread.Sleep(1500); //delay a little bit

            try
            {
                DatabaseUtil.DatabaseModel.GetProvider().CretaDatabaseIfNotExist();
                DatabaseUtil.DatabaseModel.GetConnection().Open();
                DatabaseUtil.DatabaseModel.GetConnection().Close();

                //Import tables
                DatabaseUtil.DatabaseModel.GetProvider().ImportTables();
